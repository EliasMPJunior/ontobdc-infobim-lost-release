...
This repository is one part of a multi-repo system. Know which repo you're
in and which of the others a change actually belongs to before editing —
these version numbers drift; check each repo's own `pyproject.toml`
`version` field (or, for the ontology repo, its latest `vX.Y` branch) before
trusting this table.

| Repo | What it is | WIP branch (as of this writing) |
|---|---|---|
| **`ontobdc-wip`** *(this repo)* | Offline-first semantic runtime — CLI, container/dataset storage, RDF/JSON-LD processing, the state-machine pipeline that generates Surfaces. Python package `ontobdc`. | `v0.18` |
| **`ontobdc-view`** | Browser-side presentation layer consumed by this repo at Surface-generation time: Web Component "Tiles" (closed shadow DOM), the ontology-driven Presentation Surface, i18n catalogs, standalone Page renderers (WorkStream 5W2H, file viewer). Python package `ontobdc_view`, no build step — see its own `CLAUDE.md`. | `v0.5` |
| **`infobim-wip`** | OpenBIM domain application built on top of `ontobdc` — BIM-specific entities, resource policy (IFC/DWG/etc.), CLI commands that delegate to `ontobdc`'s generic ones. Python package `infobim`. | `v0.7` |
| **`Brasidata/brasidatacenter`** | External RDF/TTL vocabulary repo (e.g. `ontology/ontobdc/domain/view.ttl`) — the canonical source for ontology terms `ontobdc`/`ontobdc-view` consume by URI. Not part of the Python/JS toolchain; changes to vocabulary terms belong there, not duplicated locally. | `v0.7` |

`ontobdc-wip` and `ontobdc-view` are separate installable packages with only
loose version pins between them (see `infobim-wip`'s `pyproject.toml`) — a
fix landing in one repo's branch does not mean it's what's actually running
wherever the pipeline is invoked from. Verify the *installed* version, not
just the repo checkout, when a reported bug doesn't reproduce.
...
- The active development branch is `v0.18`. This number drifts — check
  `pyproject.toml`'s `version` field before trusting this line, and update
  it here when it's wrong instead of leaving it stale for the next agent.
...
- **Before starting any change, `git fetch origin <branch>` and diff HEAD
  against it.** More than one agent (or the user, directly) may push to this
  branch without your session knowing — this has actually happened here.
  Never assume the checkout you started with is current.
- **Before pushing, fetch and diff again.** If origin moved while you were
  working, reconcile and re-verify before pushing; don't push blind.
...
- **Non-negotiable**: the generated Surface (`index.html`) must never read
  real file bytes/parts/hashes of any container file. Every piece of
  information shown about a file (name, size, tree position) comes
  exclusively from the RO-Crate JSON-LD embedded in the page. The only
  exception is when the user explicitly double-clicks a file to view it —
  only then may that one file's real content be read (currently via the
  standalone `onto-file-viewer.html` page, written by
  `SurfacePackagedCapability` inside the ignored `.__ontobdc__/` marker
  directory rather than the container root so it never leaks into the
  RO-Crate inventory itself, opened through `ontobdc-view`'s
  `onto-file-viewer-tile`). This applies regardless of whether the concern
  looks like "network" — containers living in cloud-synced folders
  (OneDrive Files On-Demand and equivalents) hydrate on any real file read,
  not just network access, so "no network calls" is not sufficient proof
  this rule holds. Verify with an actual file-request trace (Playwright
  `page.on("request", ...)` against the real container), not by inspecting
  code for `fetch`/`XMLHttpRequest` alone.
...
1. confirm the current repository and branch, and that it matches origin (see rule 2);
...
6. **when the user has already stated the correct fix or constraint in plain language, implement exactly that — do not treat it as one input to weigh against a theory you already have.** A precise instruction from the person who owns the system's intended contract outranks inference from reading code.
7. **when you cannot reproduce a reported bug, say so and instrument until you can — do not propose an architecture change as a guess.** A redesign is not a diagnosis. Find the exact failing line (via an actual reproduction: run it, trace it, click it) before deciding the fix needs to be bigger than that line.
