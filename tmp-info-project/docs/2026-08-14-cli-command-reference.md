...
| Route (2 forms) | 1) `storage --update` → container inferred from the current working directory<br>2) `storage --container-id <full-urn-or-short-id> --update` → explicit selection. The short id form (a bare uuid4, without the `urn:uuid:` prefix) is automatically expanded. |
| Usage | `ontobdc storage --update`<br>`ontobdc storage --container-id urn:uuid:<uuid4> --update` |
...
| Normalisation of `<container-id>` | a bare short id (a uuid4 with no `urn:` prefix) is prefixed with `urn:uuid:`; full URNs are left untouched. |
...
| Route | `storage --container <id-or-path> --create <path>` (5 positional args total). `<id-or-path>` accepts either a registered `urn:uuid:...` container id or a filesystem path; `ContainerIdStrategy` disambiguates it. |
...
| Usage | `ontobdc view`<br>`ontobdc view --container ./meu-projeto`<br>`ontobdc view --container urn:uuid:<uuid4> --language pt-br`<br>`ontobdc view --type standard --representation html` |
