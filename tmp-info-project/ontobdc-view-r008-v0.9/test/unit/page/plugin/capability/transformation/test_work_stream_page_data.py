...
from ontobdc_view.page.plugin.builder.work_stream.work_stream import (
    WorkStreamPageGenerationDataTransitionHandler,
)
...
def test_sismic_statechart_executes_page_data_capabilities(monkeypatch: Any) -> None:
    ...
    toolbar_configuration = {"@graph": [{"@id": "urn:toolbar:work-stream"}]}
    monkeypatch.setattr(
    ...
    payload = WorkStreamPageGenerationDataTransitionHandler().build_payload(
        context=FakeCliContext(),
        element_uri="urn:element",
        entity_uri="urn:entity",
        source_node={mapped_property: [{"@value": "Stream"}]},
    )
    ...
    assert payload == {
        ...
        "related_entities": [],
        "annotations": [],
        "@graph": [{mapped_property: [{"@value": "Stream"}]}],
        mapped_property: [{"@value": "Stream"}],
        "dimensions": [],
    ...
    assert executed_capabilities == [
        ...
        "DimensionCardsGeneratedCapability",
