...
# Canonical (product -> asset layout) table. Each tier maps to:
...
def _candidate_brand_roots(root_path: Optional[str]) -> List[Path]:
    ...
    ``ontobdc view`` can be launched from arbitrary shell locations, some
    ...
    to empty temp dirs. Instead we collect candidates from three tiers and
    let ``_resolve_brand_from_assets`` walk each of them upward until
    ...
    3. Directories derived from the installed ``__file__`` locations of
       ``ontobdc`` (shared runtime) and ``ontobdc_view`` (ourselves) when
       we are deployed as an editable monorepo. These paths are static
       for a given environment and therefore immune to any ``cwd`` or
       ``chdir`` shenanigans.
    ...
    for module_name in ("ontobdc", "ontobdc_view"):
        try:
...
def _resolve_brand_from_assets(root_path: Optional[str]) -> Optional[Dict[str, str]]:
    ...
    No coupling to that capability exists here: we simply scan the
    canonical project hidden directory (``.__ontobdc__``) starting from
    every candidate root returned by ``_candidate_brand_roots`` and
    walking upward to the filesystem root for each. That dual search
    (many starting points + walk up per starting point) guarantees we
    find assets placed in the workspace root even when callers cannot
    provide an explicit project path and the current working directory
    is not set to a project subtree.
...
# `ontobdc_view/component/adapter/i18n/locale/*.yaml`) that carries its
# translated strings.
...
def _project_brand(root_path: Optional[str]) -> dict:
    ...
    0. Fallback root. ``root_path`` is allowed to be ``None`` because some
       callers invoke ``component_source(tag)`` with no project hint; in
       that case we walk up from the current working directory so the
    ...
    2. Product-tier branding discovered from files in the project root
       ...
       previously downloaded ``OntoBDCBrand.svg`` / ``OntoBDCLogotype.svg``
       into ``.__ontobdc__/asset``, this tier overrides the shipped
       defaults without any Python coupling to that capability.
    ...
    # ``slogan`` is allowed to land as "" so a project can drop the
    # shipped "Data with Brains" phrase and the logo tile switches
    # cleanly to logotype-only representation. All other keys still
    # require a truthy non-empty value to be applied.
    ...
    # to the shipped ``_BRAND`` default for that key. Rationale: an old
...
class ComponentSourceAdapter(ComponentSourcePort):
    ...
    "system" Tile's JS (logo/theme/language) carries one of the
    ...
    Two locations are tried for a tag's source, in order: the per-Tile
    package layout (`component/plugin/tile/<slug>/<tag>.js`) that every Tile
    lives under, and the flat `page/plugin/asset/<tag>.js` layout that
    non-Tile system elements (e.g. `onto-presentation-surface`, the Surface
    container itself) use instead.
    """

    def component_source(self, tag: str, root_path: Optional[str] = None) -> Optional[str]:
        source = self._read_asset_source(tag)
        if source is None:
            return None
    ...
    @staticmethod
    def _read_asset_source(tag: str) -> Optional[str]:
        slug = tag.removeprefix("onto-").removesuffix("-tile")
        tile_path = files("ontobdc_view").joinpath(
            "component", "plugin", "tile", slug, f"{tag}.js"
        )
        try:
            if tile_path.is_file():
                return tile_path.read_text(encoding="utf-8")
        except (FileNotFoundError, OSError):
            pass

        try:
            return files("ontobdc_view").joinpath(
                "page", "plugin", "asset", f"{tag}.js"
