...
class DataGatheredCapability(TransformationCapability):
    ...
    def _add_surfaceable_declarations(self, graph: Graph) -> None:
        ...
        `.__ontobdc__/view/onto-file-viewer.html` page
        (`onto-file-tree-tile`'s job) instead — the one place, and the only
        moment (an explicit click), any real file content is ever read.
        """
        for entity_type in (
            _OBDC.DataContainer,
            _OBDC.FileTree,
