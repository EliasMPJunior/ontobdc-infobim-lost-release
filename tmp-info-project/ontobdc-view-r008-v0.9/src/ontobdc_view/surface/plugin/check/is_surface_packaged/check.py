...
_COMPONENT_EVENT_PROMOTER_MARKER = "window.OntoBDCComponentEventPromoter = promoter;"
...
def main(surface_path: Optional[str] = None) -> int:
    try:
        ...
        return 1
    return 0 if is_packaged_surface(document) else 1
...
def has_packaged_runtime(document: str) -> bool:
    if SurfaceDocumentAdapter.contains_external_runtime_reference(document):
        return False
    has_component_script = re.search(
        r"<script\b[^>]*\bdata-ontobdc-surface-component(?:=|\s|>)",
        document,
        re.IGNORECASE,
    ) is not None
    return has_component_script and _COMPONENT_EVENT_PROMOTER_MARKER in document
