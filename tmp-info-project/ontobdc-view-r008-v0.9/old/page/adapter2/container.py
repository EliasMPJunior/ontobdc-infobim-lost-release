...
def _render(template: str, options: "PageRuntimeOptions") -> str:
    ...
    if _GLOBAL_EVENT_LISTENER_PLACEHOLDER in rendered:
        # `global_event_listener_source()` (component/adapter/global_event_
        # listener.py, not migrated) ships the real Global Event journal
        # writer -- a set of `ontobdc.view.adapter.surface.global_event*` /
        # `ontobdc.view.domain.machine.global_event_state` modules -- into
        # Pyodide verbatim. Those modules no longer exist in the current
        # `ontobdc` package (see SurfacePackagedCapability's own docstring,
        # which already documents this exact gap). No fallback: generating
        # connection_state.js without the real listener would ship a Page
        # whose "submit" path silently does nothing useful, so this raises
        # instead of masking the gap with a stub.
        raise NotImplementedError(
            "connection_state.js requires the Global Event journal listener "
            "(component/adapter/global_event_listener.py), which has not "
            "been migrated from the legacy ontobdc.view.* package yet -- "
            "migrate it before generating this script."
        )
...
_CONNECTION_STATE_TEMPLATE = r"""(function (window) {
  ...
  // The container has to be visible to Python for the listener to write into
  ...
  async function ensureEventContainerMount(pyodide) {
    if (runtime.state.activeMountPath) return runtime.state.activeMountPath;
    const rootHandle = runtime.state.rawContainerHandle || runtime.state.datasetHandle;
    if (!rootHandle) throw new Error(t("connectFolderFirst"));
    const mountPath = GLOBAL_EVENT_MOUNT_PREFIX + Date.now();
    pyodide.FS.mkdirTree(mountPath);
    await pyodide.mountNativeFS(mountPath, rootHandle);
    runtime.state.activeMountPath = mountPath;
    return mountPath;
  }
