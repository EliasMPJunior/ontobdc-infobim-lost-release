from ontobdc.cli.adapter.response import (
    ...
    GroupedGraphCommandResponseWidgetAdapter,
...
def test_help_shaped_content_sections_a_scalar_list_and_a_dict_of_records_into_a_table() -> None:
    # A dict-of-records "Commands" section (one nested record per key)
    # renders as a table -- this is the shape a HelpCommandResponse takes
    # when a command wants to list several flags each with their own
    # id/accepts/description, as opposed to a flat {name: description}
    # map (which renders as a plain KeyValueWidget instead, see
    # cli/plugin/command/base.py's `ontobdc --help`).
...
def test_grouped_graph_response_groups_a_single_subject_star_into_one_heading() -> None:
    # Mirrors ContextGraphAdapter.load()'s output for context.ttl, which is
    # always a single :CurrentContext subject with a handful of properties
    # -- exactly the shape a node-link graph renders as overlapping boxes
    # around one hub instead of something readable.
    adapter = GroupedGraphCommandResponseWidgetAdapter()
    response = GroupedGraphCommandResponse(
        title="Context Graph",
        description="context.ttl",
        content={
            "nodes": [
                {"id": "uri:ctx", "label": ":CurrentContext", "kind": "uri"},
                {"id": "uri:exec", "label": "obdc:ExecutionContext", "kind": "uri"},
                {"id": "uri:named", "label": "owl:NamedIndividual", "kind": "uri"},
                {"id": "literal:en", "label": '"en"', "kind": "literal"},
            ],
            "edges": [
                {"source": "uri:ctx", "target": "uri:exec", "label": "rdf:type"},
                {"source": "uri:ctx", "target": "uri:named", "label": "rdf:type"},
                {"source": "uri:ctx", "target": "literal:en", "label": "obdc:contextLanguage"},
            ],
        },
    )
    ...
    # Heading (title/description) + exactly one subject group.
    assert len(widgets) == 3
    assert isinstance(widgets[1], TextWidget)
    assert widgets[1].heading == ":CurrentContext"
    assert isinstance(widgets[2], KeyValueWidget)
    assert widgets[2].pairs == [
        ("rdf:type", "obdc:ExecutionContext"),
        ("rdf:type", "owl:NamedIndividual"),
        ("obdc:contextLanguage", '"en"'),
    ]


def test_grouped_graph_response_gives_each_subject_its_own_group() -> None:
    adapter = GroupedGraphCommandResponseWidgetAdapter()
    response = GroupedGraphCommandResponse(
        content={
            "nodes": [
            ...
            "edges": [
                {"source": "a", "target": "c", "label": ":knows"},
                {"source": "b", "target": "c", "label": ":knows"},
            ],
        },
    )
    ...
    # widgets[0] is the response's own title/description heading.
    headings = [widget.heading for widget in widgets[1:] if isinstance(widget, TextWidget)]
    # ":C" is only ever an edge target, never a subject -- it must not get
    # its own group, only ":A" and ":B" (the two real subjects) do.
    assert headings == [":A", ":B"]
