...
def test_surface_transformations_build_cumulative_offline_html(tmp_path: Path) -> None:
    ...
    promoter_marker = "window.OntoBDCComponentEventPromoter = promoter;"
    assert "data-ontobdc-surface-component" in document
    assert promoter_marker in document
    assert document.index(promoter_marker) < document.index("customElements.get")
    assert re.search(
    ...
    surface_path.write_text(document.replace(promoter_marker, ""), encoding="utf-8")
    assert check_surface_packaged(str(surface_path)) != 0
