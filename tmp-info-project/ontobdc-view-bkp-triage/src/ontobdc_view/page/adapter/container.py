...
_GLOBAL_EVENT_LISTENER_PLACEHOLDER = "__ONTOBDC_BUILD_GLOBAL_EVENT_LISTENER__"
...
def _render(template: str, options: "PageRuntimeOptions") -> str:
    ...
    if _GLOBAL_EVENT_LISTENER_PLACEHOLDER in rendered:
        # The Python the Page calls directly, embedded at generation time.
        # Imported here rather than at module import so a caller that only
        # wants the chrome templates does not pay for reading the listener's
        # sources.
        ...
        rendered = rendered.replace(
            _GLOBAL_EVENT_LISTENER_PLACEHOLDER,
            json.dumps(global_event_listener_source(), ensure_ascii=False),
        )
    if "__ONTOBDC_BUILD_" in rendered:
        raise ValueError(
            "a generated container runtime script still carries an unresolved "
            "build placeholder"
        )
    return rendered
      ...
      if ((!result || !result.nodes || result.nodes.length === 0) && typeof ensurePyodide === "function") {
        try {
          ...
          if (runtime.state.activeMountPath) {
            ...
            runtime.state.activeMountHandle = null;
          }
          ...
          runtime.state.activeMountHandle = resolvedDatasetHandle;
          try {
...
_CONNECTION_STATE_TEMPLATE = r"""(function (window) {
  ...
  // Global Event listener bridge ---------------------------------------
  //
  // `submitGlobalEvent(event)` is a direct, awaitable call into Python. The
  // Page has already written and verified its data source; this hands the
  // event to the listener, which stores it under the dataset's
  // `.__ontobdc__/event/` and appends its JSON-LD to the Surface journal,
  // and resolves with the sequence the journal assigned.
  //
  // There is no queue and no daemon. The call *is* the processing: it
  // resolves as success only when the event is on disk in both places, so
  // there is no window in which a Page believes a change reached the Surface
  // and nothing has yet.
  //
  // Deliberately not the regeneration request below. That one is a single
  // overwritable file — the right shape for "something structural changed,
  // rebuild me", where only the latest signal matters. A Global Event is an
  // incremental semantic fact and never overwrites another.
  const GLOBAL_EVENT_SEQUENCE_PARAM = "_ontobdc_global_event";
  const GLOBAL_EVENT_LISTENER_SOURCE = __ONTOBDC_BUILD_GLOBAL_EVENT_LISTENER__;
  const GLOBAL_EVENT_MOUNT_PREFIX = "/ontobdc_event_container_";

  let globalEventListenerPromise = null;

  function newGlobalEventId() {
  ...
  // The dataset has to be visible to Python for the listener to write into
  // it. A Page connected through SheetJS never mounted it, so mount it here
  // rather than requiring a particular connect path to have been taken.
  //
  // What gets mounted is `datasetHandle` and only `datasetHandle`. The user
  // may well have picked a parent folder holding several datasets — that is
  // what `rawContainerHandle` keeps, and it is not this Page's dataset.
  // resolveContainerHandle()'s descent is what identified the dataset the
  // Surface was generated for, and both files the listener touches
  // (`.__ontobdc__/event/` and `index.html`) live inside it. Mounting the
  // picked folder instead would write the event into a sibling of the
  // dataset, where no Surface will ever replay it, and hand Python write
  // access to every other dataset under the selection.
  async function ensureEventContainerMount(pyodide) {
    const datasetHandle = runtime.state.datasetHandle;
    if (!datasetHandle) throw new Error(t("connectFolderFirst"));
    // An existing mount is reusable only if it is a mount *of this dataset*.
    // `activeMountPath` is a shared slot — the workbook parse, the Gantt
    // ingestion and this all write it — so the path alone says nothing about
    // which directory is behind it; `activeMountHandle` is what does.
    if (
      runtime.state.activeMountPath
      && runtime.state.activeMountHandle === datasetHandle
    ) {
      return runtime.state.activeMountPath;
    }
    if (runtime.state.activeMountPath) {
      try {
        pyodide.FS.unmount(runtime.state.activeMountPath);
      } catch (unmountError) {
        console.warn("[ontobdc] could not unmount the previous container", unmountError);
      }
      runtime.state.activeMountPath = null;
      runtime.state.activeMountHandle = null;
    }
    const mountPath = GLOBAL_EVENT_MOUNT_PREFIX + Date.now();
    pyodide.FS.mkdirTree(mountPath);
    await pyodide.mountNativeFS(mountPath, datasetHandle);
    runtime.state.activeMountPath = mountPath;
    runtime.state.activeMountHandle = datasetHandle;
    return mountPath;
  }

  async function ensureGlobalEventListener(pyodide) {
    if (!globalEventListenerPromise) {
      globalEventListenerPromise = (async () => {
        await pyodide.runPythonAsync(GLOBAL_EVENT_LISTENER_SOURCE);
        const module = pyodide.globals.get("ontobdc_global_event_listener");
        if (!module) {
          throw new Error("the Global Event listener did not install in Pyodide");
        }
        return module;
      })();
      globalEventListenerPromise.catch(function () {
        globalEventListenerPromise = null;
      });
    }
    return globalEventListenerPromise;
  }

  function syncEventFilesystem(pyodide) {
    return new Promise(function (resolve, reject) {
      pyodide.FS.syncfs(false, function (error) { error ? reject(error) : resolve(); });
    });
  }

  // Resolves with `{eventId, sequence, status}` from the listener itself.
  // Anything short of that — a failed store, a failed append, no Pyodide —
  // throws, because the Page must not tell the user the Surface knows when
  // it does not.
  async function submitGlobalEvent(event) {
    const eventId = String(event && event.eventId ? event.eventId : newGlobalEventId());
    const envelope = Object.assign({}, event, {
      eventId: eventId,
      occurredAt: event && event.occurredAt ? event.occurredAt : new Date().toISOString(),
    });

    const pyodide = await runtime.ensurePyodide();
    const mountPath = await ensureEventContainerMount(pyodide);
    const listener = await ensureGlobalEventListener(pyodide);

    const withLock = (typeof runtime.withPyodideLock === "function")
      ? runtime.withPyodideLock
      : function (task) { return task(); };

    const answer = await withLock(async function () {
      // JSON in, JSON out: one string across the boundary, no proxy to
      // destroy and no dict to marshal.
      const raw = listener.handle_global_event_json(mountPath, JSON.stringify(envelope));
      const parsed = JSON.parse(raw);
      // The listener wrote through Pyodide's filesystem; nothing is on the
      // real disk until this returns.
      await syncEventFilesystem(pyodide);
      return parsed;
    });

    if (!answer || answer.status !== "completed") {
      const error = new Error(
        (answer && answer.error) || "the Global Event listener did not complete"
      );
      error.globalEventState = answer && answer.state;
      error.globalEventTrace = (answer && answer.trace) || [];
  ...
  // The sequence rides in the URL as a revision marker: without it the
  // browser is entitled to serve the Surface from cache, or restore it from
  // the back-forward cache, and show the document as it was before the
  // append.
  function surfaceUrlForSequence(sequence) {
    const url = new URL("index.html", location.href);
    url.searchParams.set(GLOBAL_EVENT_SEQUENCE_PARAM, String(sequence));
    return url.href;
  }
  ...
  Object.assign(runtime, {
    ...
    submitGlobalEvent: submitGlobalEvent,
    newGlobalEventId: newGlobalEventId,
    surfaceUrlForSequence: surfaceUrlForSequence,
    GLOBAL_EVENT_SEQUENCE_PARAM: GLOBAL_EVENT_SEQUENCE_PARAM,
