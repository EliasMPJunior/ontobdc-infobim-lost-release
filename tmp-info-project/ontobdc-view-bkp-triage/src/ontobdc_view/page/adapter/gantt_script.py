...
"""
  ...
  function resolvePyodideLocalUrl() {
    ...
    var fallbackIndex = "https://cdn.jsdelivr.net/pyodide/v0.27.2/full/";
    var pyodideRel = "ifc_work_schedule_view/pyodide/v0.27.2/full/pyodide.js";

    // Local Pyodide files cannot be fetched from an opaque file:// origin,
    // but the CORS-enabled HTTPS distribution can. Use the CDN for the
    // complete runtime, not only its loader script.
    if (
      typeof location !== "undefined" &&
      location &&
      location.protocol === "file:"
    ) {
      return { scriptUrl: fallbackCdn, indexUrl: fallbackIndex };
    }
  ...
  async function ensurePyodide(options) {
    ...
    if (!pyodideLoadPromise) {
      pyodideLoadPromise = (async () => {
        // resolvePyodideLocalUrl() selects the CORS-enabled CDN for a
        // file:// document and the packaged runtime for HTTP. Let the
        // selected distribution decide whether it can load.
...
`;
  ...
  async function mountContainer(pyodide, handle) {
    ...
    runtime.state.activeMountHandle = handle;
    return mountPath;
  }
