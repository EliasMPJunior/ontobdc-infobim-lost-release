...
class SurfaceRdfParser:
    ...
    @classmethod
    def default_surface_layout(cls) -> List[SurfaceDefinition]:
        path = _ONTOLOGY_ADAPTER.get_ontology_path(
            "obdc_abox_surface_layouts", "default_surface_layout"
        )
        graph = Graph()
        graph.parse(path, format="turtle")
        return cls.parse_default_surface_layout(graph)

    @classmethod
    def parse_default_surface_layout(cls, graph: Graph) -> List[SurfaceDefinition]:
        return [
