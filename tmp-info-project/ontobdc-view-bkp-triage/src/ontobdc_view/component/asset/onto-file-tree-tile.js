...
// in it — a file was selected, a file's page was asked for, it entered or
// left fullscreen — and nothing else. It does not know whether the Surface,
// another Tile or the dDock cares, and it dispatches no Shared Event.
...
class OntoFileTreeTile extends HTMLElement {
  ...
  #onFullscreenChange = () => {
    ...
    this.#emitComponentEvent(isFullscreen ? "TileFullSized" : "TileRestored", {});
  };
  ...
  // `EntitySelected` — announced as a Component Event like every other
  // local occurrence. The Release 8 policy
  // (`ontology/tool/ontobdc/abox/presentation_events.ttl`) declares no
  // `view:promotesTo` for it, so the Listener promotes it to nothing and
  // no Shared Event is dispatched. That is the intended outcome of a
  // Component Event whose meaning stays local — not a reason to route it
  // around the dDock, which is exactly what the retired `entity-selected`
  // Presentation Global Event did.
  //
  ...
  #selectFile(path) {
    ...
    this.#emitComponentEvent("EntitySelected", { path, kind: "file" });
  }
  ...
  // `EntityPageOpenRequested` — this Tile asks for a file's page to be
  // opened and stops there. It does not open anything, does not know who
  // will, and does not decide that the request matters beyond itself: the
  // dDock promotes it to `EntityPageRequested`, and `onto-file-viewer-tile`
  // (the single Surface-wide file viewer) reacts to that Shared Event by
  // pointing its `<iframe>` at the standalone `onto-file-viewer.html` page
  // with `path` passed by reference — the main Surface never embeds file
  // content itself.
  #activateFile(path) {
    this.#emitComponentEvent("EntityPageOpenRequested", { path, kind: "file" });
  }
