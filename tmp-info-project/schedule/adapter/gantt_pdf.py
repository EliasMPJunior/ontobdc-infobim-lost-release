...
Standalone for now -- reads a workbook path directly via openpyxl rather
than going through ``ScheduleWorkbookAdapter`` (which resolves the
workbook through a container's ``datapackage.json`` and joins IfcTask to
IfcTaskTime via a ``TaskTime`` column; the workbooks this exporter has
been tested against use ``TaskTimeGlobalId`` instead, and some containers'
datapackages do not declare IfcTask/IfcTaskTime as resources yet). This
is meant to become the implementation behind a future ``infobim schedule
--pdf`` command -- wrap ``ScheduleGanttPdfExporter`` in a
``CliCommandPort`` at that point instead of rewriting it.

Usage:
    python3 -m infobim.schedule.adapter.gantt_pdf \\
        --workbook path/to/ifc_work_schedule.xlsx \\
        --out schedule.pdf
"""
...
class ScheduleGanttPdfExporter:
    ...
    def tasks(self) -> List[ScheduleTask]:
        ...
        for task_row in task_rows:
            time_row: Dict[str, Any] = time_rows_by_id.get(
                str(task_row.get("TaskTimeGlobalId") or ""), {}
            )
            ...
            tasks.append(
                ScheduleTask(
                    ...
                    completion=self._parse_completion(time_row.get("Completion")),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Export an IfcWorkSchedule workbook's IfcTask/IfcTaskTime "
            "sheets to a paginated PDF (task table + Gantt bars)."
        )
    )
    parser.add_argument(
        "--workbook", "-w", required=True, type=Path,
        help="Path to the ifc_work_schedule.xlsx workbook.",
    )
    parser.add_argument(
        "--out", "-o", required=True, type=Path,
        help="Path to write the generated PDF to.",
    )
    arguments: argparse.Namespace = parser.parse_args()

    exporter = ScheduleGanttPdfExporter(arguments.workbook)
    written_path: Path = exporter.export(arguments.out)
