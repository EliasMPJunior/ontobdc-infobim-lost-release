...
__all__ = [
    ...
    "parse_default_surface_layout",
    "parse_surface_definition",
]


def default_surface_layout() -> List[SurfaceDefinition]:
    """Every `view:DefaultSurfaceLayout` shipped in this package's own
    `default_surface_layout.ttl` asset — the Logo/Language/Theme
    ...
    Brasidata/brasidatacenter's `ontology/ontobdc/abox/default_surface_layout.ttl`.
    """
    return SurfaceRdfParser.default_surface_layout()


def parse_default_surface_layout(graph: Graph) -> List[SurfaceDefinition]:
    """Every `view:DefaultSurfaceLayout` subject found in an already-parsed
    rdflib Graph — the graph-taking counterpart to `default_surface_layout()`,
    used directly by tests and by any caller that already has the graph."""
    return SurfaceRdfParser.parse_default_surface_layout(graph)
