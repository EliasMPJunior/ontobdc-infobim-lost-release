...
from ontobdc_view.surface.adapter.document import LANGUAGE_PARAM, SurfaceDocumentAdapter
...
class SurfaceParametersEnsuredCapability(TransformationCapability):
    ...
    state (`SurfaceDocumentAdapter.embed_url_state_bootstrap`) together with
    the defaults resolved here. At runtime that bootstrap normalizes the
    address bar, applies the URL language before first paint, and exposes the
    single helper every Tile uses to carry the live state onto an internal
    link — so no component parses or rewrites the query string on its own.
    """
    ...
    def execute(self, context: CliContextPort) -> Dict[str, Any]:
        ...
        document = SurfaceDocumentAdapter.embed_url_state_bootstrap(document, defaults)
        document = SurfaceDocumentAdapter.set_state_marker(document, "surface_parameters_ensured")
    ...
    def resolve_defaults(
        ...
        never contradict the markup.
        """
        return {LANGUAGE_PARAM: cls._default_language(context, document)}
