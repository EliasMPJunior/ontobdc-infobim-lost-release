...
def vendor_asset_root(builder_package: str) -> Traversable:
    return files(builder_package)


def vendor_asset_source(
    name: str,
    *,
    builder_package: str,
    encoding: str = "utf-8",
    ...
    asset: Traversable = vendor_asset_root(builder_package).joinpath(
        f"{name}.js"
    )
    try:
