...
_ENTITY_PROPERTY_SET_PLACEHOLDER = "__ONTOBDC_BUILD_ENTITY_PROPERTY_SET_EVENT__"
...
class WorkStreamScriptAdapter(WorkStreamScriptPort):
    ...
    def script_source(self, name: str) -> str:
        ...
        return self._resolve_build_placeholders(builder(self))

    @staticmethod
    def _resolve_build_placeholders(source: str) -> str:
        """Substitute the ontology-resolved constants a generated script
        carries, so no IRI is written into the JavaScript by hand."""
        if _ENTITY_PROPERTY_SET_PLACEHOLDER in source:
            from ontobdc_view.component.adapter.global_event import (
                global_event_iri_for_operation,
            )

            # No fallback: `presentation_event.ttl` must declare a
            # `view:GlobalEvent` / `view:appliesOperation "set"` individual
            # for this script to be correct. Raises (ValueError, from
            # global_event_iri_for_operation) instead of writing a sentinel
            # IRI that would make dimension_card.js's inline edits post an
            # event nothing listens for.
            source = source.replace(
                _ENTITY_PROPERTY_SET_PLACEHOLDER,
                global_event_iri_for_operation("set"),
            )
        if "__ONTOBDC_BUILD_" in source:
            raise ValueError(
                "a generated WorkStream script still carries an unresolved "
                "build placeholder"
            )
        return source
  ...
  // Resolved from the ontology at generation time (`view:appliesOperation
  // "set"`), never spelled out here: the graph is what says which Global
  // Event carries a property assignment.
  runtime.ENTITY_PROPERTY_SET_EVENT = "__ONTOBDC_BUILD_ENTITY_PROPERTY_SET_EVENT__";
  ...
  async function ensurePyodide(options) {
    ...
    if (!pyodideLoadPromise) {
      pyodideLoadPromise = (async () => {
        // A file:// document may load Pyodide from a CORS-enabled HTTPS
        // distribution. Do not reject the document protocol before the
        // browser has had a chance to load that distribution.
...
`;
  ...
  // The predicate a workbook column stands for, as an IRI.
  //
  // A column label is not a predicate: "HowMuch" means nothing to a graph,
  // and the journal writer refuses it. Resolution order is the container's
  // own linkset first — it is the authority for that container — then the
  // canonical convention this Page already uses to read live values back
  // into the graph (WORKBOOK_COLUMN_TO_PROPERTY). Returns "" when neither
  // knows, and the caller then declines to emit rather than guessing.
  function workStreamPredicateFor(column) {
    const declared = runtime.state.liveColumnMappings || null;
    if (declared) {
      const mapped = declared[column];
      if (typeof mapped === "string" && mapped.trim()) return mapped.trim();
    }
    const canonical = WORKBOOK_COLUMN_TO_PROPERTY[column];
    return typeof canonical === "string" && canonical.trim() ? canonical.trim() : "";
  }

  // One saved field, one Global Event. `operations` stays an array because
  // an action that changes several properties at once is one logical edit and
  // has to reach the journal as one event with one sequence.
  //
  // Emitted only after the workbook write has completed and the value has
  // been read back: an event for a change that did not land would put the
  // Surface ahead of its own data source.
  async function emitWorkStreamFieldEvent(column, value, previousValue) {
    const predicate = workStreamPredicateFor(column);
    if (!predicate) {
      throw new Error(t("globalEventNoPredicate", { column: column }));
    }
    const entity = String(WORKSTREAM_PAYLOAD?.workstreamUri || "").trim();
    if (!entity) {
      throw new Error(t("globalEventNoEntity"));
    }

    const next = String(value ?? "");
    if (next === String(previousValue ?? "")) {
      // Nothing changed. Recording it would grow the journal and cost a
      // compaction for an edit that never happened.
      return null;
    }

    // A direct call into the Python listener. It resolves only once the
    // event is stored under the dataset's `.__ontobdc__/event/` and its
    // JSON-LD is in the Surface journal with a sequence assigned — so there
    // is no state in which this returns and the Surface does not yet know.
    // A failure propagates with the statechart state it stopped in; the
    // workbook keeps the new value either way, and the caller is told the
    // truth about the part the user is about to go and look at.
    return await runtime.submitGlobalEvent({
      event: runtime.ENTITY_PROPERTY_SET_EVENT,
      entity: entity,
      source: {
        kind: "xlsx",
        dataset: String(WORKSTREAM_PAYLOAD?.datasetFolder || ""),
        resource: "work_stream",
        entityIdentifier: String(WORKSTREAM_PAYLOAD?.elementId || ""),
        column: column,
      },
      operations: [
        { operation: "set", predicate: predicate, value: { "@value": next } },
      ],
    });
  }
  ...
  async function saveWorkStreamField(column, value) {
    ...
    if (hasSheetJS() && runtime.state.workbookFileHandle && runtime.state.workbookSheetJs) {
      ...
      const previousSheetJsValue = runtime.state.liveWorkbookRecord[column];
      runtime.state.liveWorkbookRecord[column] = value;
      return await emitWorkStreamFieldEvent(column, value, previousSheetJsValue);
    }
    ...
    runtime.state.liveWorkbookRecord = runtime.state.liveWorkbookRecord || {};
    const previousValue = runtime.state.liveWorkbookRecord[column];
    runtime.state.liveWorkbookRecord[column] = value;
    return await emitWorkStreamFieldEvent(column, value, previousValue);
  }
  ...
  function applyLiveWorkbookResult(result, options) {
    ...
    // The container's own column -> property mapping, from its ICDD linkset
    // or its facade.ttl. Only the Pyodide parse can produce it (the SheetJS
    // path cannot read Turtle), so a result without one must not erase a
    // mapping an earlier parse already established.
    if (result.mappings) runtime.state.liveColumnMappings = result.mappings;
  ...
  Object.assign(runtime, {
    ...
    workStreamPredicateFor: workStreamPredicateFor,
    emitWorkStreamFieldEvent: emitWorkStreamFieldEvent,
