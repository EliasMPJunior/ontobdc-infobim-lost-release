        ...
        for _, predicate, field_subject in graph.triples((facade_subject, None, None)):
            ...
            for _, field_predicate, obj in graph.triples((field_subject, None, None)):
                ...
                if local_name == "identifier":
                    ...
                    mapped_property = obj
                elif local_name == "mapsToDimensionKind" and isinstance(obj, URIRef):
                    dimension_kind = obj
                elif local_name == "fieldOrder":
                    field_order = obj

            if identifier is None or (
                mapped_property is None and dimension_kind is None
            ):
                continue
            field: Dict[str, Any] = {"name": str(identifier).strip()}
            if mapped_property is not None:
                field["mapped_property"] = str(mapped_property)
            if dimension_kind is not None:
                field["dimension_kind"] = str(dimension_kind)
            if field_order is not None:
                try:
                    field["field_order"] = int(str(field_order))
                except ValueError:
                    pass
            fields.append(field)
        return fields
