...
from ontobdc_view.surface.domain.port.machine import (
...
def _capability_type_for_state(state: SurfaceGenerationProcessStatePort) -> Type[CapabilityPort]:
    ...
    root_packages = (
        ("ontobdc",)
        if normalized == SurfaceGenerationProcessState.CONTAINER_HEALTHY
        else ("ontobdc_view",)
    )
    capability_type: Any = CapabilityLoader(root_packages=root_packages).get(capability_id)
...
class SurfaceGenerationStateTransitionHandler(SurfaceGenerationStateTransitionHandlerPort):
    ...
    def _get_statechart_file_path(self) -> Path:
        return StatechartLocator.locate(
            ...
            "statechart.yaml",
            statechart_package=(
                "ontobdc_view.surface.domain.machine.standard_surface_html"
            ),
        )
