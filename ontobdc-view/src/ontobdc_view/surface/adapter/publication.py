...
class SurfacePublicationAdapter:
    """Publish and validate container-backed Presentation Surface artifacts."""

    @classmethod
    def resolve_container_path(cls, context: CliContextPort) -> Path:
    ...
    def ensure_publishable(
        ...
        container_path = cls.resolve_container_path(context)
        metadata = cls._container_metadata(container_path)
    ...
    @classmethod
    def is_publishable(cls, context: CliContextPort) -> bool:
        try:
            container_path = cls.resolve_container_path(context)
            cls._container_metadata(container_path)
            descriptor = cls._load_datapackage(container_path)
            expected_paths = set(
                ContainerDataPackageSynchronizer.list_resource_paths(
                    container_path
                )
            )
            described_paths = set(
                cls._descriptor_local_paths(container_path, descriptor)
            )
    ...
    @classmethod
    def gather_view_data(
        cls,
        context: CliContextPort,
    ) -> Dict[str, Any]:
        container_path = cls.resolve_container_path(context)
        options = cls.resolve_view_options(context)
        if not cls.is_publishable(context):
            cls.ensure_publishable(context)

        source_fingerprint = cls.calculate_source_fingerprint(container_path)
        options_fingerprint = cls.calculate_options_fingerprint(options)
        metadata = cls._container_metadata(container_path)
        resources = cls._resource_records(container_path)
        ...
        payload: Dict[str, Any] = {
            ...
            "summary": {
                ...
                "total_bytes": sum(
                    int(item["bytes"]) for item in resources
                ),
            },
            "resources": resources,
        }

        data_path = cls.view_data_path(container_path)
    ...
    @classmethod
    def is_data_gathered(cls, context: CliContextPort) -> bool:
        try:
            container_path = cls.resolve_container_path(context)
            options = cls.resolve_view_options(context)
            payload = cls._load_view_data(container_path)
            return (
                payload.get("schema_version") == VIEW_SCHEMA_VERSION
                and payload.get("source_fingerprint")
                == cls.calculate_source_fingerprint(container_path)
                and payload.get("options_fingerprint")
                == cls.calculate_options_fingerprint(options)
            )
        except (OSError, TypeError, ValueError, json.JSONDecodeError):
            return False

    @classmethod
    def generate_container_view(
        cls,
        context: CliContextPort,
    ) -> Dict[str, Any]:
        container_path = cls.resolve_container_path(context)
        if not cls.is_data_gathered(context):
            payload = cls.gather_view_data(context)
        else:
            payload = cls._load_view_data(container_path)

        index_path = container_path / GENERATED_INDEX_FILE
        cls._atomic_write_text(index_path, cls._render_html(payload))
    ...
    @classmethod
    def is_generated(cls, context: CliContextPort) -> bool:
        try:
            container_path = cls.resolve_container_path(context)
            options = cls.resolve_view_options(context)
            index_path = container_path / GENERATED_INDEX_FILE
            ...
            source_fingerprint = cls.calculate_source_fingerprint(
                container_path
            )
            options_fingerprint = cls.calculate_options_fingerprint(options)
            document = index_path.read_text(encoding="utf-8")
            return (
                cls._meta_value(document, _SOURCE_FINGERPRINT_META)
                == source_fingerprint
                and cls._meta_value(document, _OPTIONS_FINGERPRINT_META)
                == options_fingerprint
                and cls.is_data_gathered(context)
            )
        except (OSError, TypeError, ValueError):
            return False

    @classmethod
    def view_data_path(cls, container_path: Path) -> Path:
        return (
            container_path
            / ".__ontobdc__"
            / VIEW_DATA_DIRECTORY
    ...
    def calculate_source_fingerprint(
        ...
        metadata_path = StorageBootstrap.get_container_storage_file_path(
            resolved
        )
        if not metadata_path.is_file():
            raise ValueError(
                f"Container metadata not found: {metadata_path}"
            )
        cls._update_file_digest(
            digest,
            metadata_path,
        ...
        for relative_path in (
            ContainerDataPackageSynchronizer.list_resource_paths(resolved)
        ):
    ...
    @classmethod
    def calculate_options_fingerprint(
        cls,
    ...
    def _update_file_digest(
        ...
        if stat_result is None:
            raise ValueError(
                f"Cannot stat resource for fingerprint: {file_path}"
            )
    ...
    def _container_metadata(
        ...
        subject = subjects[0]
        title = (
            cls._graph_text(graph, subject, DCTERMS.title)
            or container_path.name
        )
        description = cls._graph_text(
            graph,
            subject,
            _CT.description,
        )
        location = (
            cls._graph_text(graph, subject, PROV.atLocation)
            or container_path.as_uri()
        )
        identifier = (
            cls._graph_text(graph, subject, DCTERMS.identifier)
            or str(subject)
        )
            ...
            records.append(
                {
                    ...
                    "bytes": (
                        StorageBootstrap.to_extended_length_path(
                            file_path
                        ).stat().st_size
                    ),
                }
...
'''
    ...
    @classmethod
    def is_container_publishable(
        cls,
        context: CliContextPort,
    ) -> bool:
        """Return whether publication metadata matches source container files.

        The active generated Presentation Surface is an output of publication,
        not a source resource. Therefore it is ignored on both sides of the
        comparison: the current container inventory and the already synchronized
        descriptor.
        ...
        """
        try:
            container_path = cls._container_path(context)
            ...
            described_paths = set(
                cls._local_descriptor_paths(
                    container_path,
                    cls._load_descriptor(container_path),
                )
            )

            generated_surface_path = (
                cls._generated_surface_relative_path(
                    context=context,
    ...
    def _generated_surface_relative_path(
        ...
        *,
        ...
        if not raw_surface_path:
            ...
            # Publishability runs before surface_initialized, so the context may
            # not have an explicit surface_path yet even when a previous
            # index.html is already present and described by datapackage.json.
            raw_surface_path = container_path / "index.html"

        try:
            surface_path = Path(
                str(raw_surface_path)
            ).expanduser().resolve()
