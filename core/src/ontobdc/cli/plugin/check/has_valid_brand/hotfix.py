...
# import that module; instead we simply read whichever SVGs that
# capability has already downloaded into the project's hidden directory
...
def _candidate_brand_roots(resolved_root_path: Optional[Path]) -> List[Path]:
    ...
    for symmetry -- see there for the detailed rationale:
        ...
        3. Installed-package ``__file__`` dirs (ontobdc / ontobdc_view) --
           these are static for a given environment so are immune to
           cwd-chdir behaviour in build tools and daemons.
    """
...
def _discover_brand_from_assets(root_path: Path) -> Optional[Dict[str, str]]:
    ...
    for safety) looking for the canonical hidden product marker
    (``.__ontobdc__``) carrying the two required SVG assets. Mirrors
    ``ontobdc_view.component.adapter.source`` exactly so the CLI hotfix and
    the rendered Surface always agree on the effective brand, regardless of
    where the user launched the CLI from.
    """
...
def _seed_default_brand(resolved_root_path: Path) -> Dict[str, str]:
    ...
    ``slogan`` is allowed to be written as an empty string on purpose -- a
    discovered product tier may legitimately ship no slogan, and we do not
