...
# A table body cell whose entire text is one of these status words is
# tinted so a pass/fail column reads at a glance. Matched case-insensitively
# on the fully-stripped cell text, so ordinary prose is never touched.
_STATUS_CELL_COLORS: Dict[str, str] = {
    "pass": GREEN,
    "passed": GREEN,
    "ok": GREEN,
    "healthy": GREEN,
...
# A ``LABEL: value`` bullet (DETAILS cards) whose label is one of these
# renders its value dimmed -- it is a machine reference, not something the
# reader parses word by word.
_DIM_VALUE_BULLET_LABELS: frozenset = frozenset({"URI", "IRI", "URN"})
...
# cli/__init__.py's CommandResponseRenderer.response_to_markdown). Without "•" here, those
            ...
            for line_idx in range(row_height):
                ...
                for i in range(n):
                    ...
                    if i in status_color:
                        ...
                        cell = f"{status_color[i]}{cell}{RESET}"
