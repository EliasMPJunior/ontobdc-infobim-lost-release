...
class CommandTreeAdapter:
    ...
    def __init__(
        ...
        # When given, these ``(component, command_class)`` pairs replace
        # filesystem discovery entirely. A package whose commands don't
        # follow ontobdc's ``<component>/plugin/command`` + matching
        # ``METADATA.logical_component`` convention (ontobdc-dev registers
        # everything under a single ``dev`` component) can still get the
        # same tree by handing its own command list here.
        self._command_classes: Optional[
            List[Tuple[str, Type[CliCommandPort]]]
        ] = (
            list(command_classes) if command_classes is not None else None
        )
        # Alternate spellings of the executable that may appear at the head
        # of a ``usage`` string (e.g. ``"ontobdc dev"`` for usages authored
        # before the ``ontobdc-dev`` console script existed).
    ...
    def _discover_command_classes(
        ...
        if self._command_classes is not None:
            return [
                (component, command_class)
                for component, command_class in self._command_classes
                if isinstance(command_class, type)
                and command_class.METADATA.id
                not in self._excluded_command_ids
            ]
