...
def _render_response(
    ...
    pipeline, converted to a markdown representation through OntoBDC's
    shared ``CommandResponseRenderer.response_to_markdown``, then laid out
    and framed through ``TerminalSurfaceRenderer`` (the "Aggressive
    Cleanup C" terminal stack that replaced the legacy ``TerminalSurface``
    class).
    ...
    from ontobdc.view.adapter.surface import (
        TerminalSurfaceRenderer as _TSR,
    )
    from ontobdc.cli.component.widget.python import TextWidget as _TW

    # Reuse OntoBDC's shared widget->markdown dispatch and theme policy so
    # the GraphWidget / TableWidget / KeyValueWidget / CodeBlockWidget
    # branch logic stays defined once (the exact same one `ontobdc` uses).
    _renderer: CommandResponseRenderer = CommandResponseRenderer()

    body_markdown: str = _renderer.response_to_markdown(
        response,
    ...
    theme: str = _renderer.ux_theme_for(response)
